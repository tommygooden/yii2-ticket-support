...
class Content extends ContentBase
{
    ...
    public function afterSave($insert, $changedAttributes)
    {
        ...
        if ($insert) {
            ...
            // Check if the module is configured to send an email
            if ($this->getModule()->notifyByEmail) {

                // Check if the user is not the owner of the ticket
                if ($this->ticket->user_id != $this->user_id) {

                    // Check if we have an array that is not empty for the from email
                    if (!empty(\Yii::$app->getModule('support')->mailerFrom)) {
                        
                        // Push the job to the queue to send the email
                        Yii::$app->get($this->getModule()->queueComponent)->push(new SendMailJob([
                            'contentId' => $this->id,
                            'mailerFrom' => \Yii::$app->getModule('support')->mailerFrom
                        ]));
                    }
                }
            }
